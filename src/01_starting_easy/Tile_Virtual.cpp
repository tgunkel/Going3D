...
void Tile_Virtual::FIXME_paint(const double pZoomX, const double pZoomY, const double pZoomZ, const Vector3D pShift, const int FIXME_COLOR) const
{
  if(zoom_upper_left!=NULL && zoom_upper_right!=NULL && zoom_lower_left!=NULL && zoom_lower_right!=NULL)
    {
      zoom_upper_left ->FIXME_paint(pZoomX, pZoomY, pZoomZ, pShift, 0);
      zoom_upper_right->FIXME_paint(pZoomX, pZoomY, pZoomZ, pShift, 1);
      zoom_lower_left ->FIXME_paint(pZoomX, pZoomY, pZoomZ, pShift, 2);
      zoom_lower_right->FIXME_paint(pZoomX, pZoomY, pZoomZ, pShift, 3);
    }
}
